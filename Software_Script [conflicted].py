...
### ---Waiting function--- ###
def the_waiting_function():
    n = 30
    for i in range(n):
        time.sleep(1)
        sys.stdout.write('\r'+'connection to satellite...  process '+str(i)+'/'+str(n)+' '+ '{:.2f}'.format(i/n*100)+'%')
        sys.stdout.flush()
    sys.stdout.write('\r'+'connection to satellite... finished               \n')

def animated_loading():
    chars = "/—\|" 
    for char in chars:
        sys.stdout.write('\r'+'connection to satellite...'+char)
        time.sleep(.1)
        sys.stdout.flush() 
...
def main():
    ...
    waiting = threading.Thread(name='process', target=the_waiting_function)
    waiting.start()
