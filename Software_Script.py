...
class Tests:
    def __init__(self):
        self.SZ = Solarzellen()
        self.VS = Vibrationssensor()
        self.DB_Loc = Backup_Influx_Loc()
        self.DB_Ext = Backup_Influx_Ext()
        self.USB_BU = Safe_To_USB()
        self.GPS = GPS_Data()
        self.data = [1627318641.5335495, 1.0, 1.1, 1.2, 2.0, 2.1, 2.2, 3.0, 3.1,
                3.2, 4.0, 4.1, 4.2, 5.0, 5.1, 5.2, 111, 222, 333, 444, 555]

    def Test_Solarzellen(self):
        "Tests Solarzellen()"
        try:
            print(self.SZ.read_data())
        except:
            print("Couldn't execute the funtion")
            raise

    def Test_VS_Read(self):
        "Tests Vibrationssensor()"
        print('-----------------')
        values = self.VS.getacceleration()
        print(f"X: {values[0]}; Y: {values[1]}; Z: {values[2]}")
        print('-----------------')

    def Test_DB_loc_Insert(self):
        "Tests Backup_Influx_Loc()"
        self.DB_Loc.insert_data(self.data, True) #Datensatz
        self.DB_Loc.insert_data() #Fehlerhafter Datensatz

    def Test_DB_ext_Insert(self):
        "Tests Backup_Influx_Ext()"
        self.DB_Ext.insert_data(self.data, True) #Datensatz
        self.DB_Ext.insert_data() #Fehlerhafter Datensatz

    def Test_USB_BU(self):
        "Tests Safe_To_USB()"
        if (self.USB_BU.check_USB()):
            self.USB_BU.write_Backup(self.data)
    ...
    def Test_GPS(self):
        ...
        i = 0
        try:
            self.GPS.power_on(self.GPS.power_key)
            while i < 50:
                print(self.GPS.get_gps_position())
                i+=1
                time.sleep(1)
            self.GPS.power_down(self.GPS.power_key)
        except:
            if self.GPS.ser != None:
                self.GPS.ser.close()
            self.GPS.power_down(self.GPS.power_key)
            GPIO.cleanup()
        if self.GPS.ser != None:
...
if __name__ == "__main__":
    ...
    # test = Tests()
    # test.Test_VS_Read()
    # test.Test_DB_loc_Insert()
    # test.Test_DB_ext_Insert()
    # test.Test_USB_BU()
    # test.Test_GPS()
